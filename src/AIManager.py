...
class AIManager:
    ...
    def handle_command_async(self, text):
        ...
        if self.prompt_name == "prompt1":
            ...
            result = self.parse_prompt2_response(response)

        if result is None:
            self.sound_man.play_blocking("ai failed")
        else:
            self.display_man.got_ai_result(result, self.prompt_name)
    ...
    def parse_prompt2_response(self, response):
        try:
            reason = response["choices"][0]["finish_reason"]
            text = response["choices"][0]["text"]
            if reason != "stop":
